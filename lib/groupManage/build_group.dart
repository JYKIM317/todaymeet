...
DocumentReference _roomAdress = FirebaseFirestore.instance.collection('activateRoom').doc('${_user!.uid}');
DocumentReference _chatRoom = FirebaseFirestore.instance.collection('chatRoom').doc('available');
DocumentReference _madeRoom = FirebaseFirestore.instance.collection('users').doc('${_user!.uid}').collection('room').doc('made');
...
bool today = false, tomorrow = false, dat = false, inprogress = false, shown = true;
int? d_Year = today ? d_Year = DateTime.now().year
    : tomorrow ? d_Year = DateTime.now().add(Duration(days: 1)).year
    : d_Year = DateTime.now().add(Duration(days: 2)).year;
int? d_Month = today ? d_Month = DateTime.now().month
    : tomorrow ? d_Month = DateTime.now().add(Duration(days: 1)).month
    : d_Month = DateTime.now().add(Duration(days: 2)).month;
int? d_Day = today ? d_Day = DateTime.now().day
    : tomorrow ? d_Day = DateTime.now().add(Duration(days: 1)).day
...
class _MapViewState extends State<MapView> {
  ...
  CameraPosition _initialPosition = CameraPosition(target: LatLng(lat, lng), zoom: 14);
  Set<Marker> markers = {};
  ...
  static Future<String>? getPlaceAddress(lat, lng) async{
    var url = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=$lat,$lng&key=$gMapApiKey&language=ko';
    var response = await http.get(Uri.parse(url)); //google maps에서 restrictions가 android일 경우에 ip 제한 발생해버림
    List<String> place = [];
    place.add(jsonDecode(response.body)['results'][0]['address_components'][3]['long_name']);
    place.add(jsonDecode(response.body)['results'][0]['address_components'][2]['long_name']);
  ...
  Widget build(BuildContext context) {
    if(Platform.isAndroid) gMapApiKey = 'AIzaSyDsSSYPK-JDNVA04nKQC9yrcGsNzLkOfNc';
    if(Platform.isIOS) gMapApiKey = 'AIzaSyCGoLM2AaiC5sXXLMIA2BTpmL4qgj-80Tw';
    final progress = ProgressHUD.of(context);
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Colors.white,
        title: Text('만날 동네 선택', style: TextStyle(
          color: Colors.black,
          fontSize: 18.w,
        )),
        centerTitle: true,
                    ...
                    if(selectLat != null && selectLng != null ){
                      ...
                      if(lat != 37.52 && lng != 127.04){
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('위치를 선택해주세요', style: TextStyle(color: Colors.black),),
...
class _BuildGroupPageState extends State<BuildGroupPage> {
  ...
  void initState() {
    ...
    _roomAdress = FirebaseFirestore.instance.collection('activateRoom').doc('${_user!.uid}');
    _madeRoom = FirebaseFirestore.instance.collection('users').doc('${_user!.uid}').collection('room').doc('made');
    _userinfo = FirebaseFirestore.instance.collection('users').doc('${_user!.uid}');
    _chatRoom = _chatRoom = FirebaseFirestore.instance.collection('chatRoom').doc('available');
    today = false; tomorrow = false ; dat = false;
    selectedNoon = noon.last; selectedPeople = limitpeople.first;
    selectedHour = null; selectedMinute = null;
    selectedCategory = []; selectedPlace = null;
  ...
  @override
  void dispose() {
    foodState = false; alcoholState = false; cafeState = false;
  ...
  Future<List<dynamic>> getUserData()async{
    ...
    String photoUrl = userInfoData['photoUrl'] ?? 'assets/images/defaultProfile.jpg';
        ...
        header: Row(
          ...
          children: [
            ...
            Container(
                ...
                child: IconButton(
                    onPressed: (){categoryController.close();
              ...
              children: [
                  ...
                  children: [
                    TextButton(onPressed: (){
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: foodState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      alcoholState == true ?
                      {selectedCategory.remove('술자리'), alcoholState = false}
                          : {selectedCategory.add('술자리'), alcoholState = true};
                      if(selectedCategory.length >5){
                        categoryLimit(context);
                        selectedCategory.removeLast();
                        alcoholState = false;
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: alcoholState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: cafeState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      conversationState == true ?
                      {selectedCategory.remove('수다'), conversationState = false}
                          : {selectedCategory.add('수다'), conversationState = true};
                      if(selectedCategory.length >5){
                        categoryLimit(context);
                        selectedCategory.removeLast();
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: conversationState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                  ],
                ...
                Row(
                  ...
                  children: [
                    TextButton(onPressed: (){
                      movieState == true ?
                      {selectedCategory.remove('영화'), movieState = false}
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: movieState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      performanceState == true ?
                      {selectedCategory.remove('공연'), performanceState = false}
                          : {selectedCategory.add('공연'), performanceState = true};
                      if(selectedCategory.length >5){
                        categoryLimit(context);
                        selectedCategory.removeLast();
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: performanceState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      exhibitionState == true ?
                      {selectedCategory.remove('전시회'), exhibitionState = false}
                          : {selectedCategory.add('전시회'), exhibitionState = true};
                      if(selectedCategory.length >5){
                        categoryLimit(context);
                        selectedCategory.removeLast();
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: exhibitionState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                  ],
                ...
                Row(
                  ...
                  children: [
                    TextButton(onPressed: (){
                      musicState == true ?
                      {selectedCategory.remove('음악'), musicState = false}
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: musicState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      karaokeState == true ?
                      {selectedCategory.remove('노래방'), karaokeState = false}
                          : {selectedCategory.add('노래방'), karaokeState = true};
                      if(selectedCategory.length >5){
                        categoryLimit(context);
                        selectedCategory.removeLast();
                        karaokeState = false;
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: karaokeState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                  ],
                  ...
                  children: [
                    TextButton(onPressed: (){
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: tripState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      workoutState == true ?
                      {selectedCategory.remove('운동'), workoutState = false}
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: workoutState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: walkState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      driveState == true ?
                      {selectedCategory.remove('드라이브'), driveState = false}
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: driveState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                  ],
                ...
                Row(
                  ...
                  children: [
                    TextButton(onPressed: (){
                      onlineState == true ?
                      {selectedCategory.remove('온라인'), onlineState = false}
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: onlineState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: gameState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                    TextButton(onPressed: (){
                      boardgameState == true ?
                      {selectedCategory.remove('보드게임'), boardgameState = false}
                          : {selectedCategory.add('보드게임'), boardgameState = true};
                      if(selectedCategory.length >5){
                        categoryLimit(context);
                        selectedCategory.removeLast();
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: boardgameState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                  ],
                  ...
                  children: [
                    TextButton(onPressed: (){
                      ...
                      setState(() {});
                        ...
                        style: TextStyle(
                          color: etcState == true ? Color(0xFF51CF6D): Colors.grey,
                          fontSize: 18.w,
                        ))
                    ),
                  ],
                  ...
                  children:<TextSpan>[
                    TextSpan(
                      style: TextStyle(fontWeight: FontWeight.w500),
                        text:
                        '- 약속 시간은 현재 시간보다 1시간 이후로 설정 가능합니다.\n\n'
        ...
        appBar: AppBar(
          ...
          leading: IconButton(onPressed: (){
            showDialog(
              ...
              icon: Icon(Icons.close, color: Colors.black, weight: 1, size: 24.w,)
          ),
          title: Text('모임개설', style: TextStyle(color: Colors.black, fontSize: 20.w),),
        ),
                    ...
                    children: [
                      Container(
                        ...
                        child: TextField(
                          decoration: InputDecoration(
                            ...
                            labelStyle: TextStyle(color: Color(0xFF51CF6D), fontSize: 18.w)
                          ),
                          maxLength: 40,
                      ...
                      InkWell(
                            ...
                            children: [
                              ...
                              Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text('만날 동네', style: TextStyle(
                                      color: Color(0xFF51CF6D),
                                      fontSize: 14.w,
                                    )),
                                    selectedPlace != null ?
                                    Text('$selectedPlace', style: TextStyle(fontSize: 16.w),)
                                    : Text('눌러서 위치를 선택해주세요', style: TextStyle(
                                      fontSize: 16.w,
                                      color: Colors.grey,
                                    )),
                                  ]
                        ...
                        onTap: () async{
                          ...
                          if(permission == LocationPermission.denied){
                            ...
                            setState(() {});
                            if(permission == LocationPermission.deniedForever){progress?.dismiss(); return ;}
                            else if(permission == LocationPermission.unableToDetermine){progress?.dismiss(); return ;}
                            else if(permission == LocationPermission.denied){progress?.dismiss(); return ;}
                            else {
                              ...
                              await Navigator.push(context,
                                  MaterialPageRoute(builder: (context)=> ProgressHUD(child: MapView()))
                              );
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  Text('약속 시간', style: TextStyle(color: Color(0xFF51CF6D), fontSize: 16.w),),
                                ],
                              ),
                              Container(
                                child: Row(
                                  ...
                                  children: [
                                    TextButton(onPressed: (){
                                      setState(() {
                                        today = true; tomorrow = false; dat = false;
                                      });
                                    }, child: Text('오늘',style: TextStyle(
                                        color: today == true ? Color(0xFF51CF6D) : Colors.grey,
                                        fontSize: 18.w,
                                    ))),
                                    SizedBox(width: 4.w),
                                    TextButton(onPressed: (){
                                      setState(() {
                                        today = false; tomorrow = true; dat = false;
                                      });
                                    }, child: Text('내일',style: TextStyle(
                                        color: tomorrow == true ? Color(0xFF51CF6D) : Colors.grey,
                                        fontSize: 18.w,
                                    ))),
                                    SizedBox(width: 4.w),
                                    TextButton(onPressed: (){
                                      setState(() {
                                        today = false; tomorrow = false; dat = true;
                                      });
                                    }, child: Text('모레',style: TextStyle(
                                      color: dat == true ? Color(0xFF51CF6D) : Colors.grey,
                                      fontSize: 18.w,
                                    ))),
                                  ],
                                ),
                        ...
                        children: [
                          Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Text('인원 수 ', style: TextStyle(color: Color(0xFF51CF6D), fontSize: 16.w),),
                                  DropdownButton(
                                    ...
                                    iconSize: 0,
                                    underline: Container(color: Colors.transparent, height: double.minPositive, width: double.maxFinite),
                                  ),
                          ...
                          Row(children: [
                            Container(
                                ...
                                width: 50.w,
                                decoration: BoxDecoration(
                                  border: Border(bottom: BorderSide(width: 1, color: Color(0xFF51CF6D))),
                                ),
                                child: DropdownButton(
                                  ...
                                  items: noon.map((String item){
                                    return DropdownMenuItem<String>(
                                      alignment: Alignment.centerRight,
                                      child: Text('$item', style: TextStyle(fontSize: 16.w)),
                                      value: item,
                                    );
                                  }).toList(),
                                  onChanged: (dynamic value){
                      ...
                      InkWell(
                        child: Column(
                          ...
                          children: [
                            ...
                            Container(
                              alignment: Alignment.centerLeft,
                              child: selectedCategory.isEmpty ? Container(
                                  child: Text('카테고리 추가하기 +',
                                    style: TextStyle(color: Colors.grey, fontSize: 18.w)
                                  )
                              )
                                  :Container(child: Text('$categoryTextH',
                      ...
                      InkWell(
                        child: Container(
                          ...
                          child: Center(
                            child: Text('모임 만들기', style: TextStyle(fontSize: 18.w, color: Colors.white),),
                          ),
                        ),
                        onTap: ()async{
                          if(_interstitialAd != null) _interstitialAd?.show();
                          final String chatRoomName = '${DateTime.now().month.toString()}${DateTime.now().day.toString()}${DateTime.now().hour.toString()}${DateTime.now().minute.toString()}_${_user!.uid}';
                          List<dynamic> responseUserData = await getUserData();
                          String hostUID = _user!.uid;
                          List<dynamic> memberUID = [hostUID];
                          if(titleParameter!=null && selectedPlace!=null
                              && selectedHour!=null && selectedMinute!=null && (today||tomorrow||dat)==true
                              && infoParameter!=null && selectedCategory.isEmpty!=true)
                          {
                            progress?.show();
                            if(today == true){
                              if((selectedNoon=='오전'
                                  ? selectedHour == 12 ? selectedHour! - 12 : selectedHour
                                  : selectedHour == 12 ? selectedHour : selectedHour! + 12)! < DateTime.now().hour+1){
                                ...
                                if(selectedMinute! < DateTime.now().minute){
                                  ...
                                  await _roomAdress.set({
                                    ...
                                    'targetHour':selectedNoon == '오전'
                                        ? selectedHour == 12 ? selectedHour! - 12 : selectedHour
                                        : selectedHour == 12 ? selectedHour : selectedHour! + 12,
                                  ...
                                  await _chatRoom.collection(chatRoomName).doc('info').set({
                                  ...
                                  await _userinfo.collection('chat').doc(chatRoomName).set({
                                    'read':0,
                            ...
                            }else if(tomorrow == true && DateTime.now().hour >= 23){
