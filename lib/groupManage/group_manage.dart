...
DocumentReference _roominfoM = FirebaseFirestore.instance.collection('users').doc(_user!.uid).collection('room').doc('made');
DocumentReference _roominfoJ = FirebaseFirestore.instance.collection('users').doc(_user!.uid).collection('room').doc('join');
DocumentReference _roominfoA = FirebaseFirestore.instance.collection('users').doc(_user!.uid).collection('room').doc('attention');
...
var _doneRoom = FirebaseFirestore.instance.collection('users').doc(_user!.uid).collection('done');
...
class _GroupManagePageState extends State<GroupManagePage> {
  ...
  void initState() {
    ...
    _roominfoM = FirebaseFirestore.instance.collection('users').doc(_user!.uid).collection('room').doc('made');
    _roominfoJ = FirebaseFirestore.instance.collection('users').doc(_user!.uid).collection('room').doc('join');
    _roominfoA = FirebaseFirestore.instance.collection('users').doc(_user!.uid).collection('room').doc('attention');
    _doneRoom = FirebaseFirestore.instance.collection('users').doc(_user!.uid).collection('done');
    super.initState();
  }
  @override
                  ...
                  children: [
                          ...
                          TextButton(onPressed: () async{
                            if(roomStateM == false){
                              ...
                              await Navigator.push(
                                  context, MaterialPageRoute(builder: (context)=> ProgressHUD(child: BuildGroupPage()))
                              );
                              setState(() {});
                            }else{
                              showDialog(
                    ...
                    StreamBuilder<DocumentSnapshot>(
                      ...
                      builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                        ...
                        data == null ? roomStateM = false : roomStateM = true;
                        ...
                        final targetTime = DateTime(
                            data['targetYear'] == null ? 9999 : data['targetYear'] as int,
                            data['targetMonth'] == null ? 12 : data['targetMonth'] as int,
                            data['targetDay'] == null ? 31 : data['targetDay'] as int,
                            data['targetHour'] == null ? 23 : data['targetHour'] as int,
                            data['targetMinute'] == null ? 59 : data['targetMinute'] as int
                        );
                        if(targetTime.isBefore(DateTime.now()) && headcount.length == 1){
                          ...
                          FirebaseFirestore.instance
                              .collection('users').doc(_user!.uid).collection('chat').doc('${data['chatRoom']}').delete();
                        ...
                        if(targetTime.isBefore(DateTime.now()) && headcount.length >= 2 && data['inProgress'] == false){
                          ...
                          FirebaseFirestore.instance
                              .collection('inProgressRoom').doc(_user!.uid)
                              .set({
                        ...
                        if(data['inProgress'] == true && DateTime.now().difference(targetTime).inMinutes >= 60){
                          final random = Random();
                          final String roomNum = '${targetTime.year}${targetTime.month}${targetTime.day}_${random.nextInt(4294967296)}';
                          List absentMember = data['absent'];
                          for(int i=1; i<headcount.length; i++){
                            FirebaseFirestore.instance
                                .collection('users').doc('${headcount[i]}').collection('room').doc('join').delete();
                          }
                          if(absentMember.length != headcount.length){
                            for(int j=0; j<headcount.length; j++){
                              int doAbsent = absentMember.indexOf(headcount[j]);
                              if(doAbsent == -1){
                                FirebaseFirestore.instance
                                    .collection('users').doc('${headcount[j]}').update({
                                  'attend':FieldValue.increment(1),
                                });
                              }
                              FirebaseFirestore.instance
                                  .collection('users').doc('${headcount[j]}').collection('done').doc('$roomNum').set({
                                'Categories':data['Categories'],
                                'headcount':data['headcount'],
                                'hostPhotoUrl':data['hostPhotoUrl'],
                                'hostUID':data['hostUID'],
                          ...
                          FirebaseFirestore.instance.collection('users').doc('${_user!.uid}').collection('room').doc('made').delete();
                    ...
                    StreamBuilder<DocumentSnapshot>(
                        ...
                        builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                          if(snapshot.connectionState == ConnectionState.waiting) return Text('');
                          Map<String, dynamic>? uid = snapshot.data?.data() as Map<String, dynamic>?;
                          uid == null ? roomStateI = false : roomStateI = true;
                          String host = uid?['hostUID'] ?? '';
                          return uid != null ?
                          StreamBuilder<DocumentSnapshot>(
                            stream: _inProgressRoom.doc('$host').snapshots(),
                            builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                              String? categoryTextH;
                              if(snapshot.hasError) return Text('오류가 발생했습니다.');
                              if(snapshot.connectionState == ConnectionState.waiting) return Text('');
                              ...
                              if(DateTime.now().difference(targetTime).inMinutes >= 60){
                                final random = Random();
                                final String roomNum = '${targetTime.year}${targetTime.month}${targetTime.day}_${random.nextInt(4294967296)}';
                                List absentMember = data['absent'];
                                for(int i=1; i<headcount.length; i++){
                                ...
                                FirebaseFirestore.instance.collection('users').doc('$host').collection('room').doc('made').delete();
                                _inProgressRoom.doc('$host').delete();
                                roomStateI = false;
                                return SizedBox(height: 0);
                              }
                              return headcount.isNotEmpty && data != null ?
                                Container(height: 94.w, width: double.infinity,
                                  decoration: BoxDecoration(border: Border.all(color: Colors.grey, width: 1)),
                                  child: Padding(
                                    padding: EdgeInsets.all(8.w),
                                        ...
                                        Row(
                                          children: [
                                            InkWell(
                                              ...
                                              onTap: () async{
                                                await Navigator.push(context,
                                                    MaterialPageRoute(builder: (context) => MemberRoomPage(host: host, inProgress: true,)) //모임 자세히보기
                                                );
                                                setState(() {});
                                              },
                                            ),
                                            SizedBox(width: 4.w),
                                            SizedBox(width: 14.w), //관심모임엔 favorite 아이콘
                                          ],
                                        ),
                                      ],
                                    ),
                    ...
                    StreamBuilder<DocumentSnapshot>(
                        ...
                        builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                          if(snapshot.connectionState == ConnectionState.waiting) return Text('');
                          Map<String, dynamic>? uid = snapshot.data?.data() as Map<String, dynamic>?;
                          uid == null ? roomStateJ = false : roomStateJ = true;
                          String host = uid?['hostUID'] ?? '';
                          return uid != null ?
                          StreamBuilder<DocumentSnapshot>(
                            stream: _activeRoom.doc('$host').snapshots(),
                            builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                              String? categoryTextH;
                              if(snapshot.hasError) return Text('오류가 발생했습니다.');
                              if(snapshot.connectionState == ConnectionState.waiting) return Text('');
                              ...
                              if(targetTime.isBefore(DateTime.now()) && headcount.length >= 2){
                                ...
                                FirebaseFirestore.instance
                                    .collection('inProgressRoom').doc('$host')
                                    .set({
                                  'Categories':data['Categories'],
                                  'headcount':data['headcount'],
                                  'hostPhotoUrl':data['hostPhotoUrl'],
                          ...
                          ): (roomStateM==false&&roomStateJ==false&&roomStateI==false)
                              ? SizedBox(
                                  height: 52.w,
                                  child: Text(' 참가중인 모임이 없습니다.',
                                    style: TextStyle(
                                      fontSize: 22.w,
                                      color: Colors.grey,
                                    ),
                          ...
                          ListView.separated(
                            ...
                            itemBuilder: (BuildContext ctx, int idx){
                              return FutureBuilder<DocumentSnapshot?>(
                                future: _activeRoom.doc('${attention[idx]}').get(),
                                builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot?> snapshot){
                                  String? categoryTextH;
                                  if(snapshot.hasError) return Text('오류가 발생했습니다.');
                                  if(snapshot.connectionState == ConnectionState.waiting) return Text('');
                                  ...
                                  if(targetTime.isBefore(DateTime.now()) && headcount.length == 1){
                                    FirebaseFirestore.instance
                                        .collection('users').doc('${attention[idx]}').collection('chat').doc('${data['chatRoom']}').delete();
                                    FirebaseFirestore.instance.collection('chatRoom').doc('available').collection('${data['chatRoom']}').get().then((snapshot){
                                      for (DocumentSnapshot doc in snapshot.docs){
                                        doc.reference.delete();
                                      }
                                    });
                                    _activeRoom.doc('${attention[idx]}').delete();
                                    FirebaseFirestore.instance.collection('users').doc('${attention[idx]}').collection('room').doc('made').delete();
                                    return SizedBox(height: 0);
                                  }
                                  return Container(height: 94.w, width: double.infinity,
                                    decoration: BoxDecoration(border: Border.all(color: Colors.grey, width: 1)),
                                    child: Padding(
                                      padding: EdgeInsets.all(8.w),
                                      child: Row(
                                        children: [
                                          InkWell(
                                                  ...
                                                  child: Column(
                                                    ...
                                                    children: [
                                                      ...
                                                      Row(
                                                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                        children: [
                                                          Text('약속시간: ${data['targetMonth']}월${data['targetDay']}일 ${data['targetHour']}:${data['targetMinute'] == 0 ? '00' : data['targetMinute']}',
                                                              style: TextStyle(fontSize: 10.w)),
                                                          Text('인원:${headcount.length}/${data['headcount']}',
                                                              style: TextStyle(fontSize: 10.w))
                                                        ],
                                            ...
                                            onTap: () async{
                                              await Navigator.push(context,
                                                  MaterialPageRoute(builder: (context) => MemberRoomPage(host: attention[idx], inProgress: false,)) //모임 자세히보기
                                              );
                                              setState(() {});
                                            },
                                          ),
                  ...
                  children: [
                    ...
                    FutureBuilder<QuerySnapshot>(
                        ...
                        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot){
                          if(snapshot.hasError) return Text('오류가 발생했습니다.');
                          if(snapshot.connectionState == ConnectionState.waiting) return Text('');
                          final List<DocumentSnapshot> doneRoom = snapshot.data!.docs.reversed.toList();
                          ...
                          ListView.separated(
                            ...
                            itemBuilder: (BuildContext ctx, int idx){
                              return FutureBuilder<DocumentSnapshot>(
                                  future: _doneRoom.doc('${doneRoom[idx].id}').get(),
                                  builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                                    if(snapshot.hasError) return Text('오류가 발생했습니다.');
                                    if(snapshot.connectionState == ConnectionState.waiting) return Text('');
                                    String? categoryTextH;
                                    ...
                                    categoryTextH = data?['Categories'].join('  ');
                                    return Container(
                                      height: 94.w, width: double.infinity,
                                      decoration: BoxDecoration(border: Border.all(color: Colors.grey, width: 1)),
                                      child: Padding(
                                        padding: EdgeInsets.all(8.w),
                                        child: Stack(
                                            ...
                                            Row(
                                              children: [
                                                InkWell(
                                                  ...
                                                  onTap: ()async{
                                                    await Navigator.push(context,
                                                        MaterialPageRoute(builder: (context) => DoneRoomPage(
                                                          roomname: doneRoom[idx].id,
                                                        )) //모임 자세히보기
                                                    );
                                                    setState(() {});
                                                  },
