...
class _OtherProfileState extends State<OtherProfile> {
  ...
  Widget build(BuildContext context){
    Future<List<dynamic>> blockCheck()async{
      final database = FirebaseFirestore.instance.collection('users').doc(_user!.uid).collection('other').doc('block');
    ...
    Future<List<dynamic>> currentUserInfo()async{
      final database = await FirebaseFirestore.instance.collection('users').doc(_user!.uid).get();
    ...
    Future<String> messageExist()async{
      ...
      final possibility1 = await FirebaseFirestore.instance.collection('chatRoom').doc('available').collection('${_user!.uid}_${widget.user}').get();
      final response1 = possibility1.docs;
      final possibility2 = await FirebaseFirestore.instance.collection('chatRoom').doc('available').collection('${widget.user}_${_user!.uid}').get();
                            ...
                            child: Row(
                              ...
                              children: [
                                InkWell(
                                  child: Row(
                                    children: [
                                      Icon(Icons.favorite, color: Color(0xFFEF5350), size: 16.w,),
                                      SizedBox(width: 2.w),
                                      Text('관심회원 등록', style: TextStyle(fontSize: 14.w),)
                                    ],
                                  ),
                                  onTap: () async{
                                      ...
                                      showDialog(
                                          ...
                                          builder: (BuildContext context){
                                            return AlertDialog(
                                              ...
                                              content: Text("관심 목록에 대상을 추가했습니다."),
                                            );
                                          }
                                      );
                                    }
                                ...
                                InkWell(
                                  child: Row(
                                    children: [
                                      Icon(Icons.mode_comment_outlined, size: 16.w,),
                                      SizedBox(width: 2.w),
                                      Text('대화하기', style: TextStyle(fontSize: 14.w),),
                                    ],
                                  ),
                                  onTap: () async{
                                    ...
                                    if(messageRoom == 'empty'){
                                      List<dynamic> responseMyData = await currentUserInfo();
                                        ...
                                        showDialog(
                                            ...
                                            builder: (BuildContext context){
                                              return AlertDialog(
                                                ...
                                                content: Text("이번 달 남은 대화 걸기 횟수는\n${responseMyData[0]}회 입니다.\n대화하시겠습니까?"),
                                                actions: [
                                                  TextButton(
                                                      child: Text('취소하기',
                                                          style: TextStyle(
                                                  ...
                                                  TextButton(
                                                      ...
                                                      onPressed: () async{
                                                        int opportunity = responseMyData[0];
                                                        ...
                                                        showDialog(
                                                            ...
                                                            builder: (BuildContext context){
                                                              return AlertDialog(
                                                                ...
                                                                content: Text("대화방을 생성했습니다."),
                                                              );
                                                            }
                                                        );
                                                        await FirebaseFirestore.instance
                                                            .collection('chatRoom').doc('available')
                                                            .collection('${_user!.uid}_${widget.user}').doc('info')
                                                            .set({
                                                          'title':[responseMyData[1],data['username']],
                                                          'newestMessage':' ',
                                                          'member':[_user!.uid, widget.user],
                                                          'isGroup':false,
                                                          'chatRoom':'${_user!.uid}_${widget.user}',
                                                          'memberPhotoUrl':[responseMyData[2],data['photoUrl']],
                                                          'memberTokenList':[responseMyData[3],data['pushToken']],
                                                        });
                                                        await FirebaseFirestore.instance
                                                            .collection('users').doc(_user!.uid).update({
                                                          'canMessage':opportunity,
                                                        });
                                                        await FirebaseFirestore.instance
                                                            .collection('users').doc(_user!.uid)
                                                            .collection('chat').doc('${_user!.uid}_${widget.user}').set({
                                                          'read':0,
                                                          'recent':DateTime.now()
                                                        });
                                                      }
                                        ...
                                        showDialog(
                                            ...
                                            builder: (BuildContext context){
                                              return AlertDialog(
                                                ...
                                                content: Text("1:1 대화 가능 횟수가 0회 남았습니다.\n횟수는 매월 1일 초기화됩니다."),
                                              );
                                            }
                                        );
                                      }
                                      ...
                                      final roomData = await FirebaseFirestore.instance
                                          .collection('chatRoom').doc('available').collection(messageRoom).doc('info').get();
                                      List roomTitle = roomData.get('title');
                                      List memberList = roomData.get('member');
                                          ...
                                          onTap: (){
                                            Navigator.push(context,
                                                MaterialPageRoute(builder:
                                                    (context) => SimpleReview(member: widget.user, reviews: data))
                                            );
                                          },
                                        ),
                                      ),
                ...
                children: [
                  InkWell(onTap: ()async{
                    ...
                    if(blocklist.contains(widget.user)){
                      ...
                      await FirebaseFirestore.instance
                          .collection('users').doc(_user!.uid).collection('other').doc('block').update({'block':blocklist});
                      await _userinfo.collection('other').doc('block').update({'blocked':blockedlist});
                      showDialog(
                          barrierDismissible: true,
                          context: context,
                          builder: (BuildContext context){
                    ...
                    children: [
                      ...
                      SizedBox(width: 4.w),
                      Text('차단하기',
                        style: TextStyle(color: Colors.grey, fontSize: 14.w, decoration: TextDecoration.underline)),
                    ],
                  )
                  ),
                  SizedBox(width: 16.w),
                  InkWell(onTap: () {
                        ...
                        builder: (BuildContext context){
                          ...
                          return AlertDialog(
                            ...
                            content: Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                Text('신고하기', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 22.w)),
                                SizedBox(
                                  height: 24.w,
                                  width: MediaQuery.sizeOf(context).width,
                                ),
                                  ...
                                  child: SingleChildScrollView(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Container(
                                          child: Text('내용', style: TextStyle(fontSize: 14.w),),
                                          padding: EdgeInsets.only(bottom: 4.w),
                                          decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Color(0xFF51CF6D)))),
                                        ),
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    }, child: Text('취소하기', style: TextStyle(fontSize: 16.w, color: Colors.grey,),)),
                                    SizedBox(width: 24.w),
                                    TextButton(onPressed: () async{
                                      if(reportReason.trim().isNotEmpty){
                                        await FirebaseFirestore.instance.collection('report').doc('${DateTime.now()}_${widget.user}').set({
                                          'plaintiff':_user!.uid,
                                        ...
                                        showDialog(
                                            ...
                                            builder: (BuildContext context){
                                              return AlertDialog(
                                                ...
                                                content: Text("신고 내용이 없습니다."),
                                              );
                                            }
                                        );
                                      }

                                    }, child: Text('신고하기', style: TextStyle(fontSize: 16.w, color: Color(0xFFEF5350),),)),
                                  ],
                                )
                              ],
                            ),
                    ...
                    children: [
                      Icon(Icons.report, color: Colors.grey, size: 18.w,),
                      SizedBox(width: 4.w),
                      Text('신고하기',
                          style: TextStyle(color: Colors.grey, fontSize: 14.w, decoration: TextDecoration.underline)),
                    ],
