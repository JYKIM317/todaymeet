...
class MemberRoomPage extends StatefulWidget {
  ...
  const MemberRoomPage({Key? key, required this.host, required this.inProgress}) : super(key: key);
...
class _MemberRoomPageState extends State<MemberRoomPage> {
  ...
  late List<dynamic> member = [], headcount = [], headcountPhoto = [], memberToken = [];
  ...
  void initState() {
    ...
    _roomAdressA = FirebaseFirestore.instance.collection('activateRoom').doc('${widget.host}');
    _roomAdressI = FirebaseFirestore.instance.collection('inProgressRoom').doc('${widget.host}');
    _requestAdress = FirebaseFirestore.instance
        .collection('users').doc('${widget.host}').collection('room').doc('made');
    interstitialAd();
    super.initState();
  }
                            ...
                            InkWell(
                              ...
                              onTap: (){
                                Navigator.push(context,
                                    MaterialPageRoute(builder: (context) => OtherProfile(user: absentMember[index]))
                                );
                              },
                            ),
                            Row(
              ...
              child: Column(
                ...
                children: [
                  ...
                  StreamBuilder<DocumentSnapshot>(
                      stream: widget.inProgress ? _roomAdressI.snapshots() :_roomAdressA.snapshots(),
                      builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                        if(snapshot.hasError) return Text('오류가 발생했습니다.');
                        if(snapshot.connectionState == ConnectionState.waiting) return Center(child: Text(''));
                        ...
                        Center(child: Text('참가 한 방이 존재하지 않습니다.\n다시 확인해보세요',
                          style: TextStyle(
                              fontSize: 24.w,
                              fontWeight: FontWeight.w700,))
                        );
            ...
            InkWell(
              ...
              onTap: ()async{
                ...
                DocumentReference joinExist = FirebaseFirestore.instance
                    .collection('users').doc('${_user!.uid}').collection('room').doc('join');
                Future<DocumentSnapshot> existData = joinExist.get();
                dontexitState == true && targetTime.difference(DateTime.now()).inMinutes < 30 ?
                showDialog(
                    ...
                    builder: (BuildContext context){
                      return AlertDialog(
                        ...
                        content: Text("현재 신청하신 모임의 약속시간이\n30분내로 남아 들어가게 될 시\n모임탈퇴가 불가능합니다."),
                        actions: [
                          Center(
                            child: TextButton(
                                child: Text('알겠습니다',
                  ...
                  showDialog(
                      ...
                      builder: (BuildContext context){
                        return AlertDialog(
                          ...
                          content: Text("모임은 하나만 가입 가능합니다."),
                        );
                      })
                      : membercount!.length<limitmember! ? {
                    requestCheck.add(_user!.uid),
                    await _requestAdress.update({'requestUID': requestCheck}),
                    requestTime = DateTime(
                      DateTime.now().year, DateTime.now().month, DateTime.now().day,
                      DateTime.now().hour, DateTime.now().minute,
                    ),
                    await FirebaseFirestore.instance
                        .collection('users').doc('${widget.host}')
                        .collection('notification').doc('${requestTime}_${_user!.uid}').set({
                      'uid':_user!.uid,
                      'title':data['title'],
                    ...
                    await sendMessage(
                      ...
                      builder: (BuildContext context){
                        return AlertDialog(
                          ...
                          content: Text("모임 인원 충족되어 신청할 수 없습니다."),
                        );
                      });
                });
              },
            ...
            InkWell(
              ...
              onTap: ()async{
                if(targetTime.difference(DateTime.now()).inMinutes > 30){
                  final int indexToRemove = headcount.indexOf('${_user!.uid}');
                  DateTime exitTime;
                  ...
                  await FirebaseFirestore.instance
                      .collection('users').doc('${widget.host}')
                      .collection('notification').doc('${exitTime}_${_user!.uid}').set({
                    'uid':_user!.uid,
                    'title':data['title'],
                    'timeStamp':exitTime,
                    'state':'exit',
                  ...
                  await FirebaseFirestore.instance
                      .collection('users').doc('${_user!.uid}').collection('room').doc('join').delete();
                  await FirebaseFirestore.instance
              ...
              onTap: ()async{
                final int indexToRemove = requestCheck.indexOf('${_user!.uid}');
