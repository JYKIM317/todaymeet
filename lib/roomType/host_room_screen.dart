...
DocumentReference _activeRoom = FirebaseFirestore.instance.collection('activateRoom').doc(_user!.uid);
DocumentReference _inProgressRoom = FirebaseFirestore.instance.collection('inProgressRoom').doc(_user!.uid);
...
class _HostRoomPageState extends State<HostRoomPage> {
  ...
  void initState() {
    ...
    _activeRoom = FirebaseFirestore.instance.collection('activateRoom').doc(_user!.uid);
    interstitialAd();
    super.initState();
  }
      ...
      header: Row(
        ...
        children: [
          ...
          Container(
              ...
              child: IconButton(
                  onPressed: (){requestController.close();
          ...
          child: ListView.separated(
            ...
            itemBuilder: (BuildContext context, int index){
              return FutureBuilder(
                  future: FirebaseFirestore.instance.collection('users').doc('${requestUser[index]}').get(),
                        ...
                        InkWell(
                              ...
                              children: [
                                ...
                                Column(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        Text('${data['username']}',style: TextStyle(fontSize: 14.w)),
                                        SizedBox(width: 6.w),
                                        Text('${data['gender']}', style: TextStyle(color: Colors.grey, fontSize: 14.w)),
                                      ],
                                    ),
                                    Text('${data['year']}.${data['month']}.${data['day']}', style: TextStyle(fontSize: 14.w),),
                                  ],
                                ),
                              ],
                            ),
                          ...
                          onTap: (){
                            Navigator.push(context,
                                MaterialPageRoute(builder: (context) => OtherProfile(user: requestUser[index]))
                            );
                          },
                        ),
                        Row(
                          children: [
                            InkWell(
                              child: Container(
                                ...
                                child: Text('거절', style: TextStyle(color: Colors.grey, fontSize: 18.w),),
                              ),
                              onTap: () async{
                                ...
                                await FirebaseFirestore.instance
                                    .collection('users').doc('${requestUser[index]}')
                                    .collection('notification').doc('${deniedTime}_${_user!.uid}').set({
                                'uid':_user!.uid,
                                'title':roomTitle,
                                ...
                                await _roominfoM.update({'requestUID':requestUser});
                                setState(() {});
                              },
                            ),
                            SizedBox(width: 18.w),
                            InkWell(
                              child: Container(
                                ...
                                child: Text('수락', style: TextStyle(color: Colors.white, fontSize: 18.w),),
                              ),
                              onTap: () async{
                                if(member.length<limitmember){
                                  DocumentReference joinExist = FirebaseFirestore.instance
                                      .collection('users').doc('${requestUser[index]}').collection('room').doc('join');
                                  Future<DocumentSnapshot> existData = joinExist.get();
                                  String chat;
                                  DateTime acceptTime;
                                    ...
                                    showDialog(
                                        ...
                                        builder: (BuildContext context){
                                          return AlertDialog(
                                            ...
                                            content: Text("대상이 이미 가입되어있는 모임이 존재합니다."),
                                          );
                                        }
                                    ),
                                    } : {
                                      ...
                                      memberphoto.add('${data['photoUrl']}'),
                                      memberToken.add('${data['pushToken']}'),
                                      ...
                                      await FirebaseFirestore.instance
                                          .collection('users').doc('${requestUser[index]}')
                                          .collection('room').doc('join').set({'hostUID':'${_user!.uid}'}),
                                      await FirebaseFirestore.instance
                                          .collection('users').doc('${requestUser[index]}')
                                          .collection('chat').doc(chat).set({
                                        'read':0,
                                        'recent': DateTime(2099,1,1,1,1),
                                      }),
                                      await FirebaseFirestore.instance
                                          .collection('chatRoom').doc('available')
                                          .collection(chat).doc('info').update({
                                        'member':member,
                                        'memberPhotoUrl':memberphoto,
                                        'memberTokenList':memberToken,
                                          }),
                                      ...
                                      await FirebaseFirestore.instance
                                          .collection('users').doc('${requestUser[index]}')
                                          .collection('notification').doc('${acceptTime}_${_user!.uid}').set({
                                        'uid':_user!.uid,
                                        'title':roomTitle,
      ...
      header: Row(
        ...
        children: [
          ...
          Container(
              ...
              alignment: Alignment.centerRight,
              child: IconButton(
                  onPressed: (){managementController.close();
                  }, icon: Icon(Icons.close, size: 24.w,)
              )
          )
        ],
      ...
      body: SingleChildScrollView(
        ...
        child: Padding(
          ...
          child: ListView.separated(
              ...
              itemBuilder: (BuildContext context, int index){
                return FutureBuilder(
                    future: FirebaseFirestore.instance.collection('users').doc('${member[index+1]}').get(),
                    builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                      if(snapshot.hasError) return Text('오류가 발생했습니다.');
                      if(snapshot.connectionState == ConnectionState.waiting) return Center(child: Text(''));
                      Map<String, dynamic>? data = snapshot.data?.data() as Map<String, dynamic>?;
                      ...
                      return Row(
                        ...
                        children: [
                          InkWell(
                            ...
                            onTap: (){
                              Navigator.push(context,
                                  MaterialPageRoute(builder: (context) => OtherProfile(user: member[index+1]))
                              );
                            },
                          ),
                          Row(
                            children: [
                              InkWell(
                                child: Container(
                                  ...
                                  child: Text('내보내기', style: TextStyle(color: Colors.grey, fontSize: 14.w),),
                                ),
                                onTap: () async{
                                  if(targetTime.difference(DateTime.now()).inMinutes > 30){
                                    final String chat = await chatRoomName();
                                    await FirebaseFirestore.instance
                                        .collection('users').doc('${member[index+1]}')
                                        .collection('room').doc('join').delete();
                                    await FirebaseFirestore.instance
                                        .collection('users').doc('${member[index+1]}')
                                        .collection('chat').doc(chat).delete();
                                    ...
                                    await FirebaseFirestore.instance
                                        .collection('users').doc('${member[index+1]}')
                                        .collection('notification').doc('${kickTime}_${_user!.uid}').set({
                                      'uid':_user!.uid,
                                      'title':roomTitle,
                                      'timeStamp':kickTime,
                                      'state':'kick',
      ...
      header: Row(
        ...
        children: [
          ...
          Container(
              ...
              alignment: Alignment.centerRight,
              child: IconButton(
                  onPressed: (){attendController.close();
                  }, icon: Icon(Icons.close, size: 24.w,)
              )
          )
        ],
        ...
        child: Padding(
          ...
          child: StreamBuilder<DocumentSnapshot>(
            ...
            builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
              ...
              Map<String, dynamic>? roominfo = snapshot.data?.data() as Map<String, dynamic>?;
              ...
              return ListView.separated(
                ...
                itemBuilder: (BuildContext context, int index){
                  return absentMember[index] != _user!.uid ? FutureBuilder(
                      future: FirebaseFirestore.instance.collection('users').doc('${absentMember[index]}').get(),
                      builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                        if(snapshot.hasError) return Text('오류가 발생했습니다.');
                        if(snapshot.connectionState == ConnectionState.waiting) return Center(child: Text(''));
                        Map<String, dynamic>? data = snapshot.data?.data() as Map<String, dynamic>?;
                        ...
                        return Row(
                          ...
                          children: [
                            InkWell(
                              ...
                              onTap: (){
                                Navigator.push(context,
                                    MaterialPageRoute(builder: (context) => OtherProfile(user: member[index]))
                                );
                              },
                            ),
                            Row(
                              children: [
                                InkWell(
                                  child: Container(
                                    ...
                                    child: Text('출석확인', style: TextStyle(color: Colors.grey, fontSize: 14.w),),
                                  ),
                                  onTap: () async{
                                    await sendMessage(
                                        userToken: [memberToken[index+1]],
                    ...
                    StreamBuilder<DocumentSnapshot>(
                      stream: _roominfoM.snapshots(),
                        builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
                          ...
                          List<dynamic> memberTokenList = data['memberTokenList'];
                          limitmember = data['headcount'];
                          memberphoto = memberPhotoUrl;
                          memberToken = memberTokenList;
                          roomTitle = data['title'];
                          targetTime = DateTime(data['targetYear'] as int, data['targetMonth'] as int, data['targetDay'] as int, data['targetHour'] as int, data['targetMinute'] as int);
                          return Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Container(
      ...
      bottomNavigationBar:
        StreamBuilder<DocumentSnapshot>(
          ...
          builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot){
            ...
            return !data['inProgress'] ? BottomAppBar(
              height: 42.w,
              color: Color(0xFF51CF6D),
              child: Row(
                ...
                children: [
                          ...
                          builder: (BuildContext context){
                            return AlertDialog(
                              ...
                              content: Row(
                                children: [
                                  Text("정말 모임을 "),
                                  Text("해체",style: TextStyle(color: Color(0xFFEF5350)),),
                                  Text("하시겠습니까?"),
                                ],
                              ),
                              actions: [
                                ...
                                TextButton(
                                    ...
                                    onPressed: ()async{
                                      ...
                                      final String chat = await chatRoomName();
                                      if(targetTime.difference(DateTime.now()).inMinutes > 30 || member.length == 1){
                                        ...
                                        for(int i=1; i<member.length; i++){
                                          await FirebaseFirestore.instance
                                              .collection('users').doc('${member[i]}').collection('room').doc('join').delete();
                                          await FirebaseFirestore.instance
                                              .collection('users').doc('${member[i]}').collection('chat').doc(chat).delete();
                                          await FirebaseFirestore.instance
                                              .collection('users').doc('${member[i]}')
                                              .collection('notification').doc('${dismissTime}_${_user!.uid}').set({
                                            'uid':_user!.uid,
                                            'title':data['title'],
                                        ...
                                        await FirebaseFirestore.instance
                                            .collection('users').doc('${_user!.uid}').collection('chat').doc(chat).delete();
                                        await FirebaseFirestore.instance.collection('chatRoom').doc('available').collection(chat).get().then((snapshot){
                  ...
                  Container(width: 1, height: 32.w, color: Colors.white,),
                  InkWell(
                    child: SizedBox(
                      child: Text('요청확인',
                          style: TextStyle(
